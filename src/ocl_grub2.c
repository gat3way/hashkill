...
/* Crack callback */
static void ocl_grub2_crack_callback(char *line, int self)
{
    int a;
    int *found;
    int err;
	...
	memcpy(mhash,mysalt2,64);
        ...
        salt.sE=(mhash[56])|(mhash[57]<<8)|(mhash[58]<<16)|(mhash[59]<<24);
        salt.sF=(mhash[60])|(mhash[61]<<8)|(mhash[62]<<16)|(mhash[63]<<24);
...
static void ocl_grub2_callback(char *line, int self)
{
	...
	rule_offload_perform(ocl_grub2_crack_callback,self);
...
void* ocl_rule_grub2_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    worker_gen(self,ocl_grub2_callback);
...
hash_stat ocl_rule_grub2(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
    	    ...
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_grub2__%s.bin",DATADIR,pbuf);
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_grub2__%s.ptx",DATADIR,pbuf);
    ...
    for (a=0;a<nwthreads;a++)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_rule_grub2_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_grub2_callback,nwthreads,SELF_THREAD);
