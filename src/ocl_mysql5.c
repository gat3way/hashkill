...
void* ocl_bruteforce_mysql5_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "mysql5_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "mysql5_long_normal", &err );
...
void* ocl_markov_mysql5_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_mysql5_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
...
static void ocl_mysql5_callback(char *line, int self)
{
	...
	ocl_rule_opt_counts[self]=0;
	rule_offload_perform(ocl_mysql5_crack_callback,self);
...
void* ocl_rule_mysql5_thread(void *arg)
{
    cl_int err;
    unsigned char hex1[hash_ret_len];
    int a;
    ...
    rule_kernel[self] = _clCreateKernel(program[self], "mysql5", &err );
    ...
    worker_gen(self,ocl_mysql5_callback);
...
hash_stat ocl_bruteforce_mysql5(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    if (hash_list->next) 
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/amd_mysql5_long__%s.bin",DATADIR,pbuf);
    	    }
    	    else
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/amd_mysql5_long_S_%s.bin",DATADIR,pbuf);
	    }
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    if (hash_list->next) 
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/nvidia_mysql5_long__%s.ptx",DATADIR,pbuf);
    	    }
    	    else
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/nvidia_mysql5_long_S_%s.ptx",DATADIR,pbuf);
	    }
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_mysql5_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_mysql5(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_mysql5_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_mysql5(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            if (hash_list->next) 
            {
        	sprintf(kernelfile,"%s/hashkill/kernels/amd_mysql5__%s.bin",DATADIR,pbuf);
    	    }
    	    else
    	    {
        	sprintf(kernelfile,"%s/hashkill/kernels/amd_mysql5_S_%s.bin",DATADIR,pbuf);
	    }
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
            if (hash_list->next) 
            {
        	sprintf(kernelfile,"%s/hashkill/kernels/nvidia_mysql5__%s.ptx",DATADIR,pbuf);
    	    }
    	    else
    	    {
        	sprintf(kernelfile,"%s/hashkill/kernels/nvidia_mysql5_S_%s.ptx",DATADIR,pbuf);
	    }
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_mysql5_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_mysql5_callback,nwthreads,SELF_THREAD);
