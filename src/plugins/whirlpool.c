...
{
    return("whirlpool \tWHIRLPOOL plugin");
}
...
char * hash_plugin_detailed(void)
{
    return("whirlpool - Whirlpool plugin\n"
	    "------------------------\n"
	    "Use this module to crack WHIRLPOOL hashes\n"
...
hash_stat hash_plugin_parse_hash(char *hashline, char *filename)
{
    ...
    /* Hash is not 128 characters long => not a whirlpool hash */
    if (strlen(hash)!=128)
    {
	return hash_err;
    }
    ...
    hex2str(line, hash, 128);
    (void)hash_add_hash(line, 64);
    (void)hash_add_salt("");
    (void)hash_add_salt2("");
...
hash_stat hash_plugin_check_hash(const char *hash, const char *password[VECTORSIZE], const char *salt,  char *salt2[VECTORSIZE], const char *username, int *num, int threadid)
{
    ...
    (void)hash_whirlpool(password, salt2, lens);
    return hash_ok;
}
