...
{
    return("osx-old \tMacOSX <= 10.6 system passwords plugin");
}
...
char * hash_plugin_detailed(void)
{
    return("osx-old - MacOSX <=10.6 system passwords plugin\n"
	    "------------------------\n"
	    "Use this module to crack MacOSX <= 10.6 password hashes\n"
...
hash_stat hash_plugin_check_hash(const char *hash, const char *password[VECTORSIZE], const char *salt,  char *salt2[VECTORSIZE], const char *username, int *num, int threadid)
{
	...
	memcpy(psalt[a]+4,password[a],strlen(password[a]));
    }
    (void)hash_sha1_unicode((const char **)psalt, salt2, lens);
    for (a=0;a<vectorsize;a++) if (memcmp((const char *)salt2[a],hash,20)==0) 
    {
        *num=a;
        return hash_ok;
    }
