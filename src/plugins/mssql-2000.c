...
{
    return("mssql-2000 \tMicrosoft SQL Server 2000 plugin");
}
...
char * hash_plugin_detailed(void)
{
    return("mssql-2000 - Microsoft SQL Server 2000 plugin\n"
	    "------------------------\n"
	    "Use this module to crack MS-SQL 2000 pwdencrypt() hashes\n"
...
hash_stat hash_plugin_parse_hash(char *hashline, char *filename)
{
    ...
    if (strlen(hashline)>128) return hash_err;
    ...
    /* Bad constant header */
    if (!strstr(temp_str,"0x0100"))
    {
	return hash_err;
    }

    /* Bad hash */
    if (strlen(temp_str)<87)
    {
	return hash_err;
    }
    ...
    strncpy(line, line2+(len-40),40);
    line[40] = 0;
    hex2str(hash, line, 40);

    (void)hash_add_hash((char *)hash, 20);
    strncpy(line, line2+6, len-86);
    line[len-86]=0;
...
hash_stat hash_plugin_check_hash(const char *hash, const char *password[VECTORSIZE], const char *salt,  char *salt2[VECTORSIZE], const char *username, int *num, int threadid)
{
    ...
    for (a=0;a<vectorsize;a++)
    {
	passwordup[a] = strupr((char *)password[a]);
	for (cnt=0;cnt<strlen(passwordup[a]);cnt++)
	{
	    newpass[a][cnt*2] = passwordup[a][cnt];
	    newpass[a][(cnt*2)+1] = 0;
	}
	cnt *= 2;
    ...
    (void)hash_sha1_unicode((const char **)newpass, salt2, lens);
...
hash_stat hash_plugin_check_hash_dictionary(const char *hash, const char *password[VECTORSIZE], const char *salt,  char *salt2[VECTORSIZE], const char *username, int *num, int threadid)
{
    ...
    (void)hash_sha1_slow((const char **)newpass, salt2, lens);
