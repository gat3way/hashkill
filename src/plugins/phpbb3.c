...
extern char *pacrypt_private_const (const char *key[VECTORSIZE], const char *salt, char *out[VECTORSIZE], int num);
...
{
    return("phpbb3 \t\tphpBB3 hashes plugin");
}
...
char * hash_plugin_detailed(void)
{
    return("phpbb3 - phpBB3 hashes plugin\n"
	    "------------------------\n"
	    "Use this module to crack phpbb3 passwords\n"
...
hash_stat hash_plugin_parse_hash(char *hashline, char *filename)
{
	...
	if (temp_str[0]!='H') return hash_err;
    }
    else
    {
	temp_str = alloca(64);
