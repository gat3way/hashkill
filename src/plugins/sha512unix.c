...
{
    return("sha512unix \tSHA512(Unix) plugin (shadow files)");
}


char * hash_plugin_detailed(void)
{
    return("sha512unix - SHA512(Unix) plugin\n"
	    "------------------------\n"
...
hash_stat hash_plugin_parse_hash(char *hashline, char *filename)
{
    ...
    snprintf(line, HASHFILE_MAX_LINE_LENGTH*4, "%s", hashline);
    if (strstr(hashline,":"))
    {
	...
	if (temp_str) 
	{
	    ...
	    if (!temp_str2) return hash_err;
	    if ((temp_str[0]!='6')) return hash_err;
...
hash_stat hash_plugin_check_hash(const char *hash, const char *password[VECTORSIZE], const char *salt,  char *salt2[VECTORSIZE], const char *username, int *num, int threadid)
{
	...
	len[a+1]=128;
     ...
     __sha512_crypt_r(password, salt, salt2, len, vectorsize);

    b=strlen(salt);
    for (a=0;a<vectorsize;a++) if (fastcompare((const char *)salt2[a]+b,hash,64)==0)
    {
        *num=a;
        return hash_ok;
    }
