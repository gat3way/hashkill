...
{
    return("sha256unix \tSHA256(Unix) plugin (shadow files)");
}


char * hash_plugin_detailed(void)
{
    return("sha256unix - SHA256(Unix) plugin\n"
	    "------------------------\n"
	    ...
	    "Some linux distributions, OpenSolaris\n"
...
hash_stat hash_plugin_parse_hash(char *hashline, char *filename)
{
	...
	if (temp_str) 
	{
	    ...
	    if (!temp_str2) return hash_err;
	    if ((temp_str[0]!='5')) return hash_err;
