...
void* ocl_bruteforce_mscash_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "mscash_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "mscash_long_normal", &err );
...
void* ocl_markov_mscash_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
	...
	while (mylist)
	{
	    ...
	    SETUP_PARAMS();
	    image.y=(reduced_charset[a1])|(markov_charset[a2]<<8)|(markov_charset[a3]<<16)|(markov_charset[a4]<<24);
	    image.z=(reduced_charset[a5])|(0<<8);
	    image.w=0;
	    ocl_set_params(try,image,singlehash,salt,mylist,&p1,&p2,&p3,&p4,&p5,&p6,plist);
	    try++;
	    if (try==wthreads[self].loops)
	...
	while (mylist)
	{
	    ...
	    SETUP_PARAMS();
	    image.y=(reduced_charset[a1])|(markov_charset[a2]<<8)|(markov_charset[a3]<<16)|(markov_charset[a4]<<24);
	    image.z=(reduced_charset[a5])|(reduced_charset[a6]<<8)|(0<<16);
	    image.w=0;
	    ocl_set_params(try,image,singlehash,salt,mylist,&p1,&p2,&p3,&p4,&p5,&p6,plist);
	    try++;
	    if (try==wthreads[self].loops)
	...
	while (mylist)
	{
	    ...
	    SETUP_PARAMS();
	    image.y=(reduced_charset[a1])|(markov_charset[a2]<<8)|(markov_charset[a3]<<16)|(markov_charset[a4]<<24);
	    image.z=(reduced_charset[a5])|(reduced_charset[a6]<<8)|(reduced_charset[a7]<<16)|(0<<24);
	    image.w=0;
	    ocl_set_params(try,image,singlehash,salt,mylist,&p1,&p2,&p3,&p4,&p5,&p6,plist);
	    try++;
	    if (try==wthreads[self].loops)
...
/* Crack callback */
static void ocl_mscash_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
...
static void ocl_mscash_callback(char *line, int self)
{
...
void* ocl_rule_mscash_thread(void *arg)
{
    cl_int err;
    int found;
...
hash_stat ocl_bruteforce_mscash(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_mscash_long__%s.bin",DATADIR,pbuf);
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_mscash_long__%s.ptx",DATADIR,pbuf);
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_mscash_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_mscash(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_mscash_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_mscash(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            char kernelfile[255];
            _clGetDeviceInfo(device[wthreads[i].deviceid], CL_DEVICE_NAME, sizeof(pbuf),pbuf, NULL );
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_mscash__%s.bin",DATADIR,pbuf);
        ...
        else
        {
            ...
            if ((compute_capability_major==2)&&(compute_capability_minor==1)) sprintf(pbuf,"sm21");
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_mscash__%s.ptx",DATADIR,pbuf);
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_mscash_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_mscash_callback,nwthreads,SELF_THREAD);
