...
void* ocl_bruteforce_mssql_2000_thread(void *arg)
{
    int err;
...
void* ocl_markov_mssql_2000_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_mssql_2000_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
	...
	_clSetKernelArg(rule_kernel[self], 5, sizeof(cl_uint4), (void*) &singlehash);
		...
		for (c=0;c<wthreads[self].vectorsize;c++)
		{
    		    ...
    		    if (memcmp(mylist->hash, (char *)rule_ptr[self]+(e)*hash_ret_len, hash_ret_len) == 0)
    		    {
                	...
                	strcat(plain,line);
                	for (x=0;x<strlen(plain);x++) plain[x]=toupper(plain[x]);
                	pthread_mutex_lock(&crackedmutex);
                	addlist = cracked_list;
                	while (addlist)
                	{
...
static void ocl_mssql_2000_callback(char *line, int self)
{
	...
	rule_offload_perform(ocl_mssql_2000_crack_callback,self);
...
void* ocl_rule_mssql_2000_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    worker_gen(self,ocl_mssql_2000_callback);
...
hash_stat ocl_bruteforce_mssql_2000(void)
{
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_mssql_2000_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_mssql_2000(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_mssql_2000_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_mssql_2000(void)
{
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_mssql_2000_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_mssql_2000_callback,nwthreads,SELF_THREAD);
