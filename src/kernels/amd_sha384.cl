...
sha384( __global ulong4 *dst,  __global uint *inp, __global uint *sizein,  __global uint *found_ind, __global uint *bitmaps, __global uint *found,  ulong4 singlehash,uint16 str) 
{
...
dst[(get_global_id(0)*3)] = (ulong4)(A.s0,B.s0,C.s0,D.s0);  
dst[(get_global_id(0)*3)+1] = (ulong4)(E.s0,F.s0,A.s1,B.s1);
dst[(get_global_id(0)*3)+2] = (ulong4)(C.s1,D.s1,E.s1,F.s1);  
}
...
void sha512block( __global ulong2 *dst,ulong xx0,ulong xx1,ulong xx2,ulong xx3,ulong ssize,   __global uint *found_ind, __global uint *bitmaps, __global uint *found,  ulong4 singlehash,uint offset) 
{
...
dst[(get_global_id(0)*6)+offset] = (ulong2)(A,B);  
dst[(get_global_id(0)*6)+offset+1] = (ulong2)(C,D);
dst[(get_global_id(0)*6)+offset+2] = (ulong2)(E,F);

}


__kernel void  __attribute__((reqd_work_group_size(64, 1, 1))) 
sha384( __global ulong2 *dst,  __global uint *inp, __global uint *sizein,  __global uint *found_ind, __global uint *bitmaps, __global uint *found,  ulong4 singlehash,uint16 str) 
{
ulong w0,w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,w16,SIZE;
uint i,ib,ic,id;  
uint x0,x1,x2,x3,x4,x5,x6,x7;
...
sha512block(dst,w0,w1,w2,w3,SIZE,found_ind,bitmaps,found,singlehash,3);
