...
w[salt.sE>>2] |= i1<<((salt.sE&3)<<3);
w[(salt.sE+1)>>2] |= i2<<(((salt.sE+1)&3)<<3);
w[(salt.sE+2)>>2] |= i3<<(((salt.sE+2)&3)<<3);
w[(salt.sE+3)>>2] |= i4<<(((salt.sE+3)&3)<<3);
...
void sha1_saltpass_long1( __global uint4 *hashes, const uint4 input, const uint size,  __global uint4 *plains, __global uint *found,  uint4 singlehash,uint4 k, uint16 salt) 
{
