...
#define GETCHAR(buf, index) (((buf)[(index)>>2] >> (((index)&3)<<3))&255U)
...
dst[(get_global_id(0)*8)+4]=a;
dst[(get_global_id(0)*8)+5]=b;
dst[(get_global_id(0)*8)+6]=c;
dst[(get_global_id(0)*8)+7]=d;
}
