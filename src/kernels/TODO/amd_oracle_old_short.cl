...
oracle_old_short( __global uint4 *dst, uint4 input, uint size,  uint4 chbase,  __global uint *found_ind, uint16 salt, __global uint *found, __global  uint *table,  uint4 singlehash) 
{

uint2 w0,w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,chbase1,res,t0,t1;
uint id=0;
uint2 i,j,bli1,bli2,blo11,blo12,blo21,blo22,blo31,blo32,blo41,blo42;
uint2 o11,o12,o21,o22,o31,o32,o41,o42;
uint u1,u2,u3,u4,u5,u6,u7,u8,ia,ib,ic,ie;
uint ii;
uint2 l,r,t,u;
uint2 ll1,ll2;
...
ii=table[getglobalid(0)];
ia = ii&255;
ib = (ii>>8)&255;
ic = (ii>>16)&255;
ie = (ii>>24)&255;
chbase1=(uint2)(chbase.x,chbase.y);
w0 = (uint2)salt.s0;
w1 = (uint2)salt.s1;
w2 = (uint2)salt.s2;
w3 = (uint2)salt.s3;
...
if (salt.sE==5) {w2|=(ia<<24);w3=(ib<<8)|(ic<<24);w4=(ie<<8)|(chbase1<<24);}
else if (salt.sE==6) {w3=(ia<<8)|(ib<<24);w4=(ic<<8)|(ie<<24);w5=(chbase1<<8);}
else if (salt.sE==7) {w3|=(ia<<24);w4=(ib<<8)|(ic<<24);w5=(ie<<8)|(chbase1<<24);}
else if (salt.sE==8) {w4=(ia<<8)|(ib<<24);w5=(ic<<8)|(ie<<24);w6=(chbase1<<8);}
else if (salt.sE==9) {w4|=(ia<<24);w5=(ib<<8)|(ic<<24);w6=(ie<<8)|(chbase1<<24);}
else if (salt.sE==10) {w5=(ia<<8)|(ib<<24);w6=(ic<<8)|(ie<<24);w7=(chbase1<<8);}
else if (salt.sE==11) {w5|=(ia<<24);w6=(ib<<8)|(ic<<24);w7=(ie<<8)|(chbase1<<24);}
else if (salt.sE==12) {w6=(ia<<8)|(ib<<24);w7=(ic<<8)|(ie<<24);w8=(chbase1<<8);}
else if (salt.sE==13) {w6|=(ia<<24);w7=(ib<<8)|(ic<<24);w7=(ie<<8)|(chbase1<<24);}
else if (salt.sE==14) {w7=(ia<<8)|(ib<<24);w8=(ic<<8)|(ie<<24);w9=(chbase1<<8);}
else if (salt.sE==15) {w7|=(ia<<24);w8=(ib<<8)|(ic<<24);w9=(ie<<8)|(chbase1<<24);}
else if (salt.sE==16) {w8=(ia<<8)|(ib<<24);w9=(ic<<8)|(ie<<24);w10=(chbase1<<8);}
else if (salt.sE==17) {w8|=(ia<<24);w9=(ib<<8)|(ic<<24);w10=(ie<<8)|(chbase1<<24);}
else if (salt.sE==18) {w9=(ia<<8)|(ib<<24);w10=(ic<<8)|(ie<<24);w11=(chbase1<<8);}
else if (salt.sE==19) {w9|=(ia<<24);w10=(ib<<8)|(ic<<24);w11=(ie<<8)|(chbase1<<24);}
else if (salt.sE==20) {w10=(ia<<8)|(ib<<24);w11=(ic<<8)|(ie<<24);w12=(chbase1<<8);}
else if (salt.sE==21) {w10|=(ia<<24);w11=(ib<<8)|(ic<<24);w12=(ie<<8)|(chbase1<<24);}
else if (salt.sE==22) {w11=(ia<<8)|(ib<<24);w12=(ic<<8)|(ie<<24);w13=(chbase1<<8);}
else if (salt.sE==23) {w11|=(ia<<24);w12=(ib<<8)|(ic<<24);w13=(ie<<8)|(chbase1<<24);}
...
dst[(getglobalid(0))] = (uint4)(blo11.x,blo12.x,blo11.y,blo12.y);
