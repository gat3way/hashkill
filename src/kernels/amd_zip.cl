...
#define CRC_UPDATE_BYTE(crc, b) \
 ((uint) (((crctable[((crc) ^ (b)) & 0xFF] ^ ((crc) >> 8)))))
...
void zip( __global uint4 *dst,  __global uint *input, __global uint *size,  __global uint *found_ind, __global uint *found, uint16 salt)
{

__local uint crctable[256];
uint ic;
uint key0 = 305419896L;
uint key1 = 591751049L;
uint key2 = 878082192L;
uint k0,k1,k2,c;
uint kk0,kk1,kk2;
...
uint password[8];
uint bt,t;
uint verifier=(uint)0;

password[0]=input[get_global_id(0)*8];
password[1]=input[get_global_id(0)*8+1];
password[2]=input[get_global_id(0)*8+2];
password[3]=input[get_global_id(0)*8+3];
password[4]=input[get_global_id(0)*8+4];
password[5]=input[get_global_id(0)*8+5];
password[6]=input[get_global_id(0)*8+6];
password[7]=input[get_global_id(0)*8+7];
...
verifier+=(uint)(1);
dst[(get_global_id(0))] = (uint4)(kk0,kk1,kk2,verifier);
