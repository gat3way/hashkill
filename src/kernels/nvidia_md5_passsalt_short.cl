...
md5_passsalt_short( __global uint4 *dst, uint4 input, uint size,  uint8 chbase,  __global uint *found_ind, uint16 salt, __global uint *found, __global  uint *table,  uint4 singlehash) 
{  
...
uint4 x0,x1,x2,x3; 
uint4 x4,x5,x6,x7,x8,x9,x10,x11,x12,chbase1; 
...
{
    x0 |= (ib<<16)|(ic<<24);
    x1 |= id|(ie<<8)|(chbase1<<16);
}
else if (size==4)
{
    x0 |= (ib<<24);
    x1 |= ic|(id<<8)|(ie<<16)|(chbase1<<24);
}
else if (size==5)
{
    x1 |= (ib)|(ic<<8)|(id<<16)|(ie<<24);
    x2 |= (chbase1);
}
else if (size==6)
{
...
{
    x1 |= (ib<<16)|(ic<<24);
    x2 |= id|(ie<<8)|(chbase1<<16);
}
else if (size==8)
{
    x1 |= (ib<<24);
    x2 |= ic|(id<<8)|(ie<<16)|(chbase1<<24);
}
else if (size==9)
{
    x2 |= (ib)|(ic<<8)|(id<<16)|(ie<<24);
    x3 |= (chbase1);
}
else if (size==10)
{
...
if (all((uint4)singlehash.x !=a+mCa)) return;
...
uint x0,x1,x2,x3; 
uint x4,x5,x6,x7,x8,x9,x10,x11,x12,chbase1; 
...
MD5STEP_ROUND4_NULL(b, c, d, a, mAC60, S44);
MD5STEP_ROUND4 (a, b, c, d, mAC61, x4, S41);
if (((uint)singlehash.x !=a+mCa)) return;
MD5STEP_ROUND4_NULL (d, a, b, c, mAC62, S42);
MD5STEP_ROUND4 (c, d, a, b, mAC63, x2, S43);
MD5STEP_ROUND4 (b, c, d, a, mAC64, x9, S44);
