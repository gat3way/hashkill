...
/* Crack callback */
static void ocl_osx_ml_crack_callback(char *line, int self)
{
    int a;
    int *found;
    int err;
	...
	strncpy(hex1,mylist->salt+64,8);
	iterations = atoi(hex1);
	for (a=0;a<64;a++) mysalt[a] = tolower(mysalt[a]);
	hex2str(mysalt2,mysalt,64);
	memset(mhash,0,32);
	memcpy(mhash,mysalt2,32);
	...
	_clSetKernelArg(rule_kernelpre1[self], 2, sizeof(cl_mem), (void*) &rule_images4_buf[self]);
	...
	_clSetKernelArg(rule_kernelbl1[self], 2, sizeof(cl_mem), (void*) &rule_images4_buf[self]);
	...
	_clSetKernelArg(rule_kernellast[self], 2, sizeof(cl_mem), (void*) &rule_images4_buf[self]);
	_clSetKernelArg(rule_kernellast[self], 3, sizeof(cl_mem), (void*) &rule_found_buf[self]);
	_clSetKernelArg(rule_kernellast[self], 4, sizeof(cl_mem), (void*) &rule_found_ind_buf[self]);
	_clSetKernelArg(rule_kernellast[self], 5, sizeof(cl_ulong8), (void*) &singlehash);
	...
	for (a=1;a<iterations;a+=100)
	{
	    ...
	    if (rule_found_ind[self][a]==1)
	    {
    		...
    		if (memcmp(mylist->hash, (char *)rule_ptr[self]+(a)*hash_ret_len1, hash_ret_len1) == 0)
    		{
            	    int flag = 0;
...
static void ocl_osx_ml_callback(char *line, int self)
{
	...
	rule_offload_perform(ocl_osx_ml_crack_callback,self);
...
void* ocl_rule_osx_ml_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    worker_gen(self,ocl_osx_ml_callback);
...
hash_stat ocl_rule_osx_ml(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
    	    ...
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_osxml__%s.bin",DATADIR,pbuf);
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_osxml__%s.ptx",DATADIR,pbuf);
    ...
    for (a=0;a<nwthreads;a++)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_rule_osx_ml_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_osx_ml_callback,nwthreads,SELF_THREAD);
