...
static void ocl_execute(cl_command_queue queue, cl_kernel kernel, size_t *global_work_size, size_t *local_work_size, int charset_size, cl_mem found_buf, cl_mem hashes_buf, cl_mem plains_buf, char *plains, char * hashes,int self, cl_uint16 *p1,cl_uint16 *p2,cl_uint16 *p3,cl_uint16 *p4)
{
	...
	for (try=0;try<16;try++)
	{
	    ...
	    if (*found>0) 
	    {
    		...
    		_clEnqueueWriteBuffer(queue, plains_buf, CL_FALSE, 0, hash_ret_len*8*MAXFOUND, plains, 0, NULL, NULL);
...
void* ocl_bruteforce_md5_thread(void *arg)
{
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "md5_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "md5_long_normal", &err );
    ...
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "md5_long_double8", &err );
    else  kernel = _clCreateKernel(program[self], "md5_long_normal8", &err );
    _clSetKernelArg(kernel, 0, sizeof(cl_mem), (void*) &hashes_buf);
    _clSetKernelArg(kernel, 1, sizeof(cl_uint), (void*) &csize);
    _clSetKernelArg(kernel, 2, sizeof(cl_mem), (void*) &plains_buf);
    _clSetKernelArg(kernel, 3, sizeof(cl_mem), (void*) &bitmaps_buf);
	...
	if (attack_over!=0) goto out;
        if (!hash_list->next)
        {
            ...
            D=(D-0x10325476);
            REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC64);
            REVERSE_MD5STEP(C, D, A, B, (bruteforce_charset[a5]|(0x80<<8)), 	S43, AC63);//x2
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC62);
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC61);
    	    REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC60);
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC59);
            ...
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC50);
	    REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC49);
	    B = ROTATE_RIGHT((B - C), 23) - (bruteforce_charset[a5]|(0x80<<8)) - 0xc4ac5665; //x2
	    image.x=0;
	    image.y=bruteforce_charset[a1]|(bruteforce_charset[a2]<<8)|(bruteforce_charset[a3]<<16)|(bruteforce_charset[a4]<<24);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            D=(D-0x10325476);
            REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC64);
            REVERSE_MD5STEP(C, D, A, B, (bruteforce_charset[a5]|(bruteforce_charset[a6]<<8)|(0x80<<16)), 	S43, AC63);//x2
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC62);
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC61);
    	    REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC60);
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC59);
            ...
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC50);
	    REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC49);
	    B = ROTATE_RIGHT((B - C), 23) - (bruteforce_charset[a5]|(bruteforce_charset[a6]<<8)|(0x80<<16)) - 0xc4ac5665; //x2
	    image.x=0;
	    image.y=bruteforce_charset[a1]|(bruteforce_charset[a2]<<8)|(bruteforce_charset[a3]<<16)|(bruteforce_charset[a4]<<24);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            D=(D-0x10325476);
            REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC64);
            REVERSE_MD5STEP(C, D, A, B, (unsigned int)(bruteforce_charset[a5]|(bruteforce_charset[a6]<<8)|(bruteforce_charset[a7]<<16)|(0x80<<24)), 	S43, AC63);//x2
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC62);
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC61);
    	    REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC60);
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC59);
            ...
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC50);
	    REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC49);
	    B = ROTATE_RIGHT((B - C), 23) - (unsigned int)(bruteforce_charset[a5]|(bruteforce_charset[a6]<<8)|(bruteforce_charset[a7]<<16)|(0x80<<24)) - 0xc4ac5665; //x2
	    image.x=0;
	    image.y=bruteforce_charset[a1]|(bruteforce_charset[a2]<<8)|(bruteforce_charset[a3]<<16)|(bruteforce_charset[a4]<<24);
	    image.z=bruteforce_charset[a5]|(bruteforce_charset[a6]<<8)|(bruteforce_charset[a7]<<16)|(0x80<<24);image.w=0;
	    image.x = ROTATE_RIGHT((C - D), 16) - 0x1fa27cf8;
	    image.y+=AC2;image.z+=AC3;
            singlehash.x=A;singlehash.y=B;singlehash.z=C;singlehash.w=D;
    	    ocl_set_params(try,image,singlehash,&p1,&p2,&p3,&p4);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            D=(D-0x10325476);
            REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC64);
            REVERSE_MD5STEP(C, D, A, B, (bruteforce_charset[a5]|(bruteforce_charset[a6]<<8)|(bruteforce_charset[a7]<<16)|(bruteforce_charset[a8]<<24)), S43, AC63);//x2
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC62);
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC61);
    	    REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC60);
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC59);
            ...
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC50);
	    REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC49);
	    B = ROTATE_RIGHT((B - C), 23) - (bruteforce_charset[a5]|(bruteforce_charset[a6]<<8)|(bruteforce_charset[a7]<<16)|(bruteforce_charset[a8]<<24)) - 0xc4ac5665; //x2
	    image.x=0;
	    image.y=bruteforce_charset[a1]|(bruteforce_charset[a2]<<8)|(bruteforce_charset[a3]<<16)|(bruteforce_charset[a4]<<24);
	    image.z=bruteforce_charset[a5]|(bruteforce_charset[a6]<<8)|(bruteforce_charset[a7]<<16)|(bruteforce_charset[a8]<<24);image.w=0x80;
	    image.x = ROTATE_RIGHT((C - D), 16) - 0x1fa27cf8;
	    image.y+=AC2;image.z+=AC3;
            singlehash.x=A;singlehash.y=B;singlehash.z=C;singlehash.w=D;
    	    ocl_set_params(try,image,singlehash,&p1,&p2,&p3,&p4);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
    	    ...
    	    REVERSE_MD5STEP(B, C, D, A, tmp, 	S44, AC56);//x1
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC55);
	    tmp = bruteforce_charset[a9]|(0x80<<8);
            REVERSE_MD5STEP(D, A, B, C, tmp, 	S42, AC54);//x3
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC53);
            REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC52);
    	    REVERSE_MD5STEP(C, D, A, B, 96,	S43, AC51);
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC50);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
    	    ...
    	    REVERSE_MD5STEP(B, C, D, A, tmp, 	S44, AC56);//x1
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC55);
	    tmp = bruteforce_charset[a9]|(bruteforce_charset[a10]<<8)|(0x80<<16);
            REVERSE_MD5STEP(D, A, B, C, tmp, 	S42, AC54);//x3
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC53);
            REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC52);
    	    REVERSE_MD5STEP(C, D, A, B, 96,	S43, AC51);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
    	    ...
    	    REVERSE_MD5STEP(B, C, D, A, tmp, 	S44, AC56);//x1
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC55);
	    tmp = bruteforce_charset[a9]|(bruteforce_charset[a10]<<8)|(bruteforce_charset[a11]<<16)|(0x80<<24);
            REVERSE_MD5STEP(D, A, B, C, tmp, 	S42, AC54);//x3
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC53);
            REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC52);
    	    REVERSE_MD5STEP(C, D, A, B, 96,	S43, AC51);
...
void* ocl_markov_md5_thread(void *arg)
{
	...
	{
    	    image.x=0;image.y=(reduced_charset[a1])|(0x80<<8);image.z=0;image.w=0;
    	    image.y+=AC2;image.z+=AC3;
    	    ocl_set_params(try,image,singlehash,&p1,&p2,&p3,&p4);
	}
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC58);
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC57);
            tmp = (reduced_charset[a1])|(markov_charset[a2]<<8)|(0x80<<16);
    	    REVERSE_MD5STEP(B, C, D, A, tmp, 	S44, AC56);//x1
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC55);
            REVERSE_MD5STEP(D, A, B, C, 0, 	S42, AC54);//x3
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC53);
	...
	{
	    image.x=0;
	    image.y=((reduced_charset[a1]&255)|(markov_charset[a2]<<8)|(0x80<<16));
	    image.z=0;image.w=0;
    	    image.y+=AC2;image.z+=AC3;
    	    ocl_set_params(try,image,singlehash,&p1,&p2,&p3,&p4);
	}
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC58);
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC57);
            tmp = reduced_charset[a1]|(markov_charset[a2]<<8)|(markov_charset[a3]<<16)|(0x80<<24);
    	    REVERSE_MD5STEP(B, C, D, A, tmp, 	S44, AC56);//x1
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC55);
            REVERSE_MD5STEP(D, A, B, C, 0, 	S42, AC54);//x3
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC53);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC58);
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC57);
            tmp = reduced_charset[a1]|(markov_charset[a2]<<8)|(markov_charset[a3]<<16)|(markov_charset[a4]<<24);
    	    REVERSE_MD5STEP(B, C, D, A, tmp, 	S44, AC56);//x1
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC55);
            REVERSE_MD5STEP(D, A, B, C, 0, 	S42, AC54);//x3
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC53);
	...
	if (attack_over!=0) goto out;
        if (!hash_list->next)
        {
            ...
            D=(D-0x10325476);
            REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC64);
            REVERSE_MD5STEP(C, D, A, B, (markov_charset[a5]|(0x80<<8)), 	S43, AC63);//x2
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC62);
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC61);
    	    REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC60);
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC59);
            ...
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC50);
	    REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC49);
	    B = ROTATE_RIGHT((B - C), 23) - (markov_charset[a5]|(0x80<<8)) - 0xc4ac5665; //x2
	    image.x=0;
	    image.y=reduced_charset[a1]|(markov_charset[a2]<<8)|(markov_charset[a3]<<16)|(markov_charset[a4]<<24);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            D=(D-0x10325476);
            REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC64);
            REVERSE_MD5STEP(C, D, A, B, (markov_charset[a5]|(markov_charset[a6]<<8)|(0x80<<16)), 	S43, AC63);//x2
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC62);
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC61);
    	    REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC60);
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC59);
            ...
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC50);
	    REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC49);
	    B = ROTATE_RIGHT((B - C), 23) - (markov_charset[a5]|(markov_charset[a6]<<8)|(0x80<<16)) - 0xc4ac5665; //x2
	    image.x=0;
	    image.y=reduced_charset[a1]|(markov_charset[a2]<<8)|(markov_charset[a3]<<16)|(markov_charset[a4]<<24);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            D=(D-0x10325476);
            REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC64);
            REVERSE_MD5STEP(C, D, A, B, (unsigned int)(markov_charset[a5]|(markov_charset[a6]<<8)|(markov_charset[a7]<<16)|(0x80<<24)), 	S43, AC63);//x2
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC62);
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC61);
    	    REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC60);
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC59);
            ...
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC50);
	    REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC49);
	    B = ROTATE_RIGHT((B - C), 23) - (unsigned int)(markov_charset[a5]|(markov_charset[a6]<<8)|(markov_charset[a7]<<16)|(0x80<<24)) - 0xc4ac5665; //x2
	    image.x=0;
	    image.y=reduced_charset[a1]|(markov_charset[a2]<<8)|(markov_charset[a3]<<16)|(markov_charset[a4]<<24);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            D=(D-0x10325476);
            REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC64);
            REVERSE_MD5STEP(C, D, A, B, (markov_charset[a5]|(markov_charset[a6]<<8)|(markov_charset[a7]<<16)|(markov_charset[a8]<<24)), S43, AC63);//x2
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC62);
            REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC61);
    	    REVERSE_MD5STEP(B, C, D, A, 0,      S44, AC60);
            REVERSE_MD5STEP(C, D, A, B, 0,      S43, AC59);
            ...
            REVERSE_MD5STEP(D, A, B, C, 0,      S42, AC50);
	    REVERSE_MD5STEP(A, B, C, D, 0,      S41, AC49);
	    B = ROTATE_RIGHT((B - C), 23) - (markov_charset[a5]|(markov_charset[a6]<<8)|(markov_charset[a7]<<16)|(markov_charset[a8]<<24)) - 0xc4ac5665; //x2
	    image.x=0;
	    image.y=reduced_charset[a1]|(markov_charset[a2]<<8)|(markov_charset[a3]<<16)|(markov_charset[a4]<<24);
...
/* Crack callback */
static void ocl_md5_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
	...
	case 3:
	    ...
	    break;
	case 4:
	    addline2[self].sD=strlen(line);
	    addline2[self].s4=line[0]|(line[1]<<8)|(line[2]<<16)|(line[3]<<24);
	    addline2[self].s5=line[4]|(line[5]<<8)|(line[6]<<16)|(line[7]<<24);
	    addline2[self].s6=line[8]|(line[9]<<8)|(line[10]<<16)|(line[11]<<24);
	    addline2[self].s7=line[12]|(line[13]<<8)|(line[14]<<16)|(line[15]<<24);
	    break;
	case 5:
	    addline2[self].sE=strlen(line);
	    addline2[self].s8=line[0]|(line[1]<<8)|(line[2]<<16)|(line[3]<<24);
	    addline2[self].s9=line[4]|(line[5]<<8)|(line[6]<<16)|(line[7]<<24);
	    addline2[self].sA=line[8]|(line[9]<<8)|(line[10]<<16)|(line[11]<<24);
	    addline2[self].sB=line[12]|(line[13]<<8)|(line[14]<<16)|(line[15]<<24);
	    break;
	case 6:
	    addline3[self].sC=strlen(line);
	    addline3[self].s0=line[0]|(line[1]<<8)|(line[2]<<16)|(line[3]<<24);
	    addline3[self].s1=line[4]|(line[5]<<8)|(line[6]<<16)|(line[7]<<24);
	    addline3[self].s2=line[8]|(line[9]<<8)|(line[10]<<16)|(line[11]<<24);
	    addline3[self].s3=line[12]|(line[13]<<8)|(line[14]<<16)|(line[15]<<24);
	    break;
	case 7:
	    addline3[self].sD=strlen(line);
	    addline3[self].s4=line[0]|(line[1]<<8)|(line[2]<<16)|(line[3]<<24);
	    addline3[self].s5=line[4]|(line[5]<<8)|(line[6]<<16)|(line[7]<<24);
	    addline3[self].s6=line[8]|(line[9]<<8)|(line[10]<<16)|(line[11]<<24);
	    addline3[self].s7=line[12]|(line[13]<<8)|(line[14]<<16)|(line[15]<<24);
	    break;
    }
    ...
    if ((line[0]==0)||(ocl_rule_opt_counts[self]>=wthreads[self].vectorsize))
    {
        ...
        _clSetKernelArg(rule_kernel[self], 9, sizeof(cl_uint16), (void*) &addline3[self]);
...
static void ocl_md5_callback(char *line, int self)
{
	...
	ocl_rule_opt_counts[self]=0;
	rule_offload_perform(ocl_md5_crack_callback,self);
...
void* ocl_rule_md5_thread(void *arg)
{
    cl_int err;
    unsigned char hex1[hash_ret_len];
    int a;
    ...
    rule_kernel[self] = _clCreateKernel(program[self], "md5", &err );
    ...
    bzero(&rule_ptr[self][0],ocl_rule_workset[self]*hash_ret_len*wthreads[self].vectorsize);
    ...
    _clEnqueueWriteBuffer(rule_oclqueue[self], rule_found_ind_buf[self], CL_TRUE, 0, 4, rule_found_ind[self], 0, NULL, NULL);

    /* Create a dummy bitmaps buffer if singlehash */
    if (hash_list->next) rule_bitmap_buf = _clCreateBuffer(context[self], CL_MEM_READ_ONLY|CL_MEM_USE_HOST_PTR, 256*256*32*4*8, bitmaps, &err );
    else rule_bitmap_buf = _clCreateBuffer(context[self], CL_MEM_READ_ONLY, 4, NULL, &err );
    rule_images_buf[self] = _clCreateBuffer(context[self], CL_MEM_READ_ONLY, ocl_rule_workset[self]*MAX, NULL, &err );
    rule_sizes_buf[self] = _clCreateBuffer(context[self], CL_MEM_READ_ONLY, ocl_rule_workset[self]*sizeof(cl_uint), NULL, &err );
    ...
    worker_gen(self,ocl_md5_callback);
...
hash_stat ocl_bruteforce_md5(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	else
    	{
    	    ...
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/nvidia_md5_long_S_%s.ptx",DATADIR,pbuf);
	    }
...
hash_stat ocl_rule_md5(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        else
        {
            ...
            if (hash_list->next) 
            {
        	sprintf(kernelfile,"%s/hashkill/kernels/nvidia_md5__%s.ptx",DATADIR,pbuf);
    	    }
    	    else
    	    {
        	sprintf(kernelfile,"%s/hashkill/kernels/nvidia_md5_S_%s.ptx",DATADIR,pbuf);
	    }
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_md5_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_md5_callback,nwthreads,SELF_THREAD);
