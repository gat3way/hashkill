...
#define SETUP_PARAMS() { \
        ...
        singlehash.w-=H3; \
        hex2str(hex1,mylist->salt,8); \
        hex1[4]=0; \
        memcpy(&image.x,hex1,4); \
    }



static void ocl_set_params(int loopnr, cl_ulong4 param1, cl_ulong4 param2,struct hash_list_s *list,cl_ulong16 *p1, cl_ulong16 *p2,struct hash_list_s *plist[4])
{
    switch (loopnr)
    {
	case 0:
...
static void ocl_execute(cl_command_queue queue, cl_kernel kernel, size_t *global_work_size, size_t *local_work_size, int charset_size, cl_mem found_buf, cl_mem hashes_buf, cl_mem plains_buf, char *plains, char * hashes,int self, cl_ulong16 *p1,cl_ulong16 *p2,struct hash_list_s *list[4])
{
    int err;
    int *found;
    int try;
    ...
    _clSetKernelArg(kernel, 5, sizeof(cl_ulong16), (void*) p1);
    _clSetKernelArg(kernel, 6, sizeof(cl_ulong16), (void*) p2);
...
void* ocl_bruteforce_osxlion_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "osxlion_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "osxlion_long_normal", &err );
...
void* ocl_markov_osxlion_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_osxlion_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
	...
	salt=0;
        ...
        hex1[4]=0;
        memcpy(&salt,hex1,4);
	_clSetKernelArg(rule_kernel2[self], 5, sizeof(cl_ulong), (void*) &salt);
	_clSetKernelArg(rule_kernel[self], 6, sizeof(cl_ulong), (void*) &salt);
        memcpy(hex1,mylist->hash,8); 
        memcpy(&singlehash.x, hex1, 8); 
        memcpy(hex1,mylist->hash+8,8); 
        memcpy(&singlehash.y, hex1, 8); 
...
static void ocl_osxlion_callback(char *line, int self)
{
	...
	rule_offload_perform(ocl_osxlion_crack_callback,self);
...
void* ocl_rule_osxlion_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    rule_kernel[self] = _clCreateKernel(program[self], "osxlion", &err );
    ...
    worker_gen(self,ocl_osxlion_callback);
...
hash_stat ocl_bruteforce_osxlion(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_osxlion_long__%s.bin",DATADIR,pbuf);
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_osxlion_long__%s.ptx",DATADIR,pbuf);
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_osxlion_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_osxlion(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_osxlion_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_osxlion(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            char kernelfile[255];
            _clGetDeviceInfo(device[wthreads[i].deviceid], CL_DEVICE_NAME, sizeof(pbuf),pbuf, NULL );
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_osxlion__%s.bin",DATADIR,pbuf);
        ...
        else
        {
            ...
            if ((compute_capability_major==2)&&(compute_capability_minor==1)) sprintf(pbuf,"sm21");
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_osxlion__%s.ptx",DATADIR,pbuf);
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_osxlion_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_osxlion_callback,nwthreads,SELF_THREAD);
