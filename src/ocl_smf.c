...
static cl_uint16 setup_salt(char *salt, char *plain)
{
    ...
    bzero(salt2,64);
    strcpy(salt2,salt);
    strcpy(salt2+strlen(salt)+4,plain);
    salt2[strlen(salt2)+4+strlen(plain)]=0x80;
    ...
    t.sE=strlen(salt);
    t.sF=(strlen(salt)+strlen(plain)+4)*8;
    return t;
}
...
void* ocl_bruteforce_smf_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "sha1_saltpass_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "sha1_saltpass_long_normal", &err );
...
void* ocl_markov_smf_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_smf_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
...
static void ocl_smf_callback(char *line, int self)
{
    rule_counts[self][0]++;
	...
	rule_offload_perform(ocl_smf_crack_callback,self);
...
void* ocl_rule_smf_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    rule_kernel[self] = _clCreateKernel(program[self], "sha1_saltpass", &err );
    ...
    worker_gen(self,ocl_smf_callback);
...
hash_stat ocl_bruteforce_smf(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_sha1_saltpass_long__%s.bin",DATADIR,pbuf);
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_sha1_saltpass_long__%s.ptx",DATADIR,pbuf);
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_smf_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_smf(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_smf_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_smf(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            char kernelfile[255];
            _clGetDeviceInfo(device[wthreads[i].deviceid], CL_DEVICE_NAME, sizeof(pbuf),pbuf, NULL );
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_sha1_saltpass__%s.bin",DATADIR,pbuf);
        ...
        else
        {
            ...
            if ((compute_capability_major==2)&&(compute_capability_minor==1)) sprintf(pbuf,"sm21");
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_sha1_saltpass__%s.ptx",DATADIR,pbuf);
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_smf_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_smf_callback,nwthreads,SELF_THREAD);
