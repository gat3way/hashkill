...
/* Hash reversal constants/macros */
...
void* ocl_bruteforce_ntlm_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "ntlm_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "ntlm_long_normal", &err );
	...
	memcpy(hex1,mylist->hash+4,4);
        ...
        tmp = ((image.w>>16)&255)|(((image.w>>24)&255)<<16);
        REVERSE_MD4STEP_ROUND3 (C, D, A, B, tmp, S33);
        REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
        tmp = ((image.y>>16)&255)|(((image.y>>24)&255)<<16); //x3
        REVERSE_MD4STEP_ROUND3 (A, B, C, D, tmp, S31);
        REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
        tmp = ((image.z>>16)&255)|(((image.z>>24)&255)<<16);
        REVERSE_MD4STEP_ROUND3 (C, D, A, B, tmp, S33);
        REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
	REVERSE_MD4STEP_ROUND3 (A, B, C, D, 0,      S31);
	...
	if (!hash_list->next)
	{
    	    image.x=0;
    	    image.y=(bruteforce_charset[a1])|(bruteforce_charset[a2]<<8)|(bruteforce_charset[a3]<<16)|(bruteforce_charset[a4]<<24);
    	    image.z=(bruteforce_charset[a5])|(bruteforce_charset[a6]<<8)|(bruteforce_charset[a7]<<16)|(bruteforce_charset[a7]<<24);
    	    image.w=(0x80);
    	    mylist = hash_list;
    	    memcpy(hex1,mylist->hash,4);
    	    unsigned int A,B,C,D;
...
void* ocl_markov_ntlm_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_ntlm_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
...
static void ocl_ntlm_callback(char *line, int self)
{
	...
	ocl_rule_opt_counts[self]=0;
	rule_offload_perform(ocl_ntlm_crack_callback,self);
...
void* ocl_rule_ntlm_thread(void *arg)
{
    cl_int err;
    unsigned char hex1[hash_ret_len];
    int a;
    ...
    worker_gen(self,ocl_ntlm_callback);
    return hash_ok;
}
...
hash_stat ocl_bruteforce_ntlm(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    if (hash_list->next) 
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/amd_ntlm_long__%s.bin",DATADIR,pbuf);
    	    }
    	    else
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/amd_ntlm_long_S_%s.bin",DATADIR,pbuf);
	    }
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    if (hash_list->next) 
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/nvidia_ntlm_long__%s.ptx",DATADIR,pbuf);
    	    }
    	    else
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/nvidia_ntlm_long_S_%s.ptx",DATADIR,pbuf);
	    }
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_ntlm_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_ntlm(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_ntlm_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_ntlm(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            if (hash_list->next) 
            {
        	sprintf(kernelfile,"%s/hashkill/kernels/amd_ntlm__%s.bin",DATADIR,pbuf);
    	    }
    	    else
    	    {
        	sprintf(kernelfile,"%s/hashkill/kernels/amd_ntlm_S_%s.bin",DATADIR,pbuf);
	    }
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
            if (hash_list->next) 
            {
        	sprintf(kernelfile,"%s/hashkill/kernels/nvidia_ntlm__%s.ptx",DATADIR,pbuf);
    	    }
    	    else
    	    {
        	sprintf(kernelfile,"%s/hashkill/kernels/nvidia_ntlm_S_%s.ptx",DATADIR,pbuf);
	    }
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_ntlm_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_ntlm_callback,nwthreads,SELF_THREAD);
