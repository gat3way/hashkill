...
static void ocl_set_params(int loopnr, cl_uint4 param1, cl_uint4 param2,cl_uint16 param3,cl_uint16 salt,struct hash_list_s *list,cl_uint16 *p1, cl_uint16 *p2, cl_uint16 *p3, cl_uint16 *p4, cl_uint16 *p5, cl_uint16 *p6,struct hash_list_s *plist[2])
{
    switch (loopnr)
    {
	case 0:
...
static cl_uint16 setup_aessalt(char *hexsalt)
{
    cl_uint16 salt;
    unsigned char *block = (unsigned char *)(hexsalt);
...
static void ocl_get_cracked(cl_command_queue queuein,cl_mem plains_buf, char *plains, cl_mem hashes_buf, char *hashes, int numfound, int vsize, int hashlen, struct hash_list_s *list)
{
    ...
    for (b=0;b<vsize;b++)
    if (memcmp("\x08\x08\x08\x08\x08\x08\x08\x08", (char *)hashes+(a*vsize+b)*16+8, 8) == 0)
    {
    	int flag = 0;
...
void* ocl_bruteforce_o5logon_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "o5logon_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "o5logon_long_normal", &err );
	...
	ocl_set_params(try,image,singlehash,setup_aessalt(mylist->hash+16),salt,mylist,&p1,&p2,&p3,&p4,&p5,&p6,plist);
	try++;
...
void* ocl_markov_o5logon_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
hash_stat ocl_bruteforce_o5logon(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_o5logon_long__%s.bin",DATADIR,pbuf);
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_o5logon_long__%s.ptx",DATADIR,pbuf);
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_o5logon_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_o5logon(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_o5logon_thread, &worker_thread_keys[a]);
    }
