...
static void init_markov()
{
    ...
    for (a=0;a<strlen(markov_charset);a++) markov_charset[a]=toupper(markov_charset[a]);
...
static void init_bruteforce_long()
{
    ...
    for (a=0;a<strlen(bruteforce_charset);a++) bruteforce_charset[a]=toupper(bruteforce_charset[a]);
    bitmaps = malloc(256*256*32*8*4);
	...
	for (try=0;try<4;try++)
	{
	    ...
	    if (*found>0) 
	    {
    		...
    		*found = 0;
    	    ...
    	    _clEnqueueUnmapMemObject(queue,found_buf,(void *)found,0,NULL,NULL);
    	    wthreads[self].tries += (charset_size*charset_size*charset_size*charset_size*wthreads[self].loops)/4;
	}
    }
...
void* ocl_bruteforce_lm_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "lm_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "lm_long_normal", &err );
   ...
   if (a==1)
   {
	...
	if (attack_over!=0) goto out;
    	...
    	image.x=0;
    	image.y=bruteforce_charset[a1]|(bruteforce_charset[a2]<<8)|(bruteforce_charset[a3]<<16)|(bruteforce_charset[a4]<<24);
    	image.z=(bruteforce_charset[a5])|(0<<8);
    	image.w=0;
    	ocl_set_params(try,image,singlehash,&p1,&p2,&p3,&p4);
	...
	if (attack_over!=0) goto out;
    	...
    	image.x=0;
    	image.y=bruteforce_charset[a1]|(bruteforce_charset[a2]<<8)|(bruteforce_charset[a3]<<16)|(bruteforce_charset[a4]<<24);
    	image.z=(bruteforce_charset[a5])|(bruteforce_charset[a6]<<8)|(0<<16);
    	image.w=0;
    	ocl_set_params(try,image,singlehash,&p1,&p2,&p3,&p4);
	...
	if (attack_over!=0) goto out;
    	...
    	image.x=0;
    	image.y=bruteforce_charset[a1]|(bruteforce_charset[a2]<<8)|(bruteforce_charset[a3]<<16)|(bruteforce_charset[a4]<<24);
    	image.z=(bruteforce_charset[a5])|(bruteforce_charset[a6]<<8)|(bruteforce_charset[a7]<<16)|(0<<24);
    	image.w=0;
    	ocl_set_params(try,image,singlehash,&p1,&p2,&p3,&p4);
...
void* ocl_markov_lm_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
   ...
   if (a==1)
   {
	...
	if (attack_over!=0) goto out;
    	...
    	image.x=0;
    	image.y=reduced_charset[a1]|(markov_charset[a2]<<8)|(markov_charset[a3]<<16)|(markov_charset[a4]<<24);
    	image.z=(markov_charset[a5])|(markov_charset[a6]<<8)|(0<<16);
    	image.w=0;
    	ocl_set_params(try,image,singlehash,&p1,&p2,&p3,&p4);
	...
	if (attack_over!=0) goto out;
    	...
    	image.x=0;
    	image.y=reduced_charset[a1]|(markov_charset[a2]<<8)|(markov_charset[a3]<<16)|(markov_charset[a4]<<24);
    	image.z=(markov_charset[a5])|(markov_charset[a6]<<8)|(markov_charset[a7]<<16)|(0<<24);
    	image.w=0;
    	ocl_set_params(try,image,singlehash,&p1,&p2,&p3,&p4);
...
/* Crack callback */
static void ocl_lm_crack_callback(char *line, int self)
{
    int a,b,c,e,f;
    int *found;
    int err;
                ...
                for (c=0;c<wthreads[self].vectorsize;c++)
                {
                    ...
                    if (hash_index[rule_ptr[self][e*hash_ret_len]&255][rule_ptr[self][e*hash_ret_len+1]&255].nodes)
                    {
                        ...
                        while (mylist)
                        {
                            ...
                            {
                                int flag = 0;
                                strcpy(plain,&rule_images[self][0]+(a*16));
...
static void ocl_lm_callback(char *line, int self)
{
    if ((rule_counts[self][0]==-1)&&(line[0]==0x01)) return;
    rule_counts[self][0]++;
    rule_sizes[self][rule_counts[self][0]] = strlen(line);
    strcpy(&rule_images[self][0]+(rule_counts[self][0]*16),line);

    if ((rule_counts[self][0]>=(ocl_rule_workset[self]-1))||(line[0]==0x01))
    {

	_clEnqueueWriteBuffer(rule_oclqueue[self], rule_images_buf[self], CL_FALSE, 0, ocl_rule_workset[self]*16, rule_images[self], 0, NULL, NULL);
	...
	ocl_rule_opt_counts[self]=0;
	rule_offload_perform(ocl_lm_crack_callback,self);
    	bzero(&rule_images[self][0],ocl_rule_workset[self]*16);
...
void* ocl_rule_lm_thread(void *arg)
{
    cl_int err;
    unsigned char hex1[hash_ret_len];
    int a;
    ...
    rule_kernel[self] = _clCreateKernel(program[self], "lm", &err );
    ...
    rule_images_buf[self] = _clCreateBuffer(context[self], CL_MEM_READ_WRITE, ocl_rule_workset[self]*16, NULL, &err );
    ...
    rule_images[self]=malloc(ocl_rule_workset[self]*16);
    bzero(&rule_images[self][0],ocl_rule_workset[self]*16);
    ...
    worker_gen(self,ocl_lm_callback);
...
hash_stat ocl_bruteforce_lm(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    if (hash_list->next) 
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/amd_lm_long__%s.bin",DATADIR,pbuf);
    	    }
    	    else
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/amd_lm_long_S_%s.bin",DATADIR,pbuf);
	    }
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    if (hash_list->next) 
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/nvidia_lm_long__%s.ptx",DATADIR,pbuf);
    	    }
    	    else
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/nvidia_lm_long_S_%s.ptx",DATADIR,pbuf);
	    }
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_lm_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_lm(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_lm_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_lm(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            if (hash_list->next) 
            {
        	sprintf(kernelfile,"%s/hashkill/kernels/amd_lm__%s.bin",DATADIR,pbuf);
    	    }
    	    else
    	    {
        	sprintf(kernelfile,"%s/hashkill/kernels/amd_lm_S_%s.bin",DATADIR,pbuf);
	    }
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
            if (hash_list->next) 
            {
        	sprintf(kernelfile,"%s/hashkill/kernels/nvidia_lm__%s.ptx",DATADIR,pbuf);
    	    }
    	    else
    	    {
        	sprintf(kernelfile,"%s/hashkill/kernels/nvidia_lm_S_%s.ptx",DATADIR,pbuf);
	    }
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_lm_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_lm_callback,nwthreads,SELF_THREAD);
