...
#define SETUP_PARAMS() { \
        ...
        memcpy(&A,mylist->hash,4); \
        memcpy(&B,mylist->hash+4,4); \
        memcpy(&C,mylist->hash+8,4); \
        memcpy(&D,mylist->hash+12,4); \
...
void* ocl_bruteforce_osx_old_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "osx_old_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "osx_old_long_normal", &err );
...
void* ocl_markov_osx_old_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_osx_old_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
	...
	salt.s0=salt.s1=salt.s2=salt.s3=salt.s4=salt.s5=salt.s6=salt.s7=salt.sF=0;
        hex2str(hex1,mylist->salt,8);
        hex1[4]=0;
	salt.s0=hex1[0]|(hex1[1]<<8)|(hex1[2]<<16)|(hex1[3]<<24);
...
static void ocl_osx_old_callback(char *line, int self)
{
    rule_counts[self][0]++;
	...
	rule_offload_perform(ocl_osx_old_crack_callback,self);
...
void* ocl_rule_osx_old_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    rule_kernel[self] = _clCreateKernel(program[self], "osx_old", &err );
    ...
    worker_gen(self,ocl_osx_old_callback);
...
hash_stat ocl_bruteforce_osx_old(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_osx_old_long__%s.bin",DATADIR,pbuf);
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_osx_old_long__%s.ptx",DATADIR,pbuf);
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_osx_old_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_osx_old(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_osx_old_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_osx_old(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            char kernelfile[255];
            _clGetDeviceInfo(device[wthreads[i].deviceid], CL_DEVICE_NAME, sizeof(pbuf),pbuf, NULL );
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_osx_old__%s.bin",DATADIR,pbuf);
        ...
        else
        {
            ...
            if ((compute_capability_major==2)&&(compute_capability_minor==1)) sprintf(pbuf,"sm21");
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_osx_old__%s.ptx",DATADIR,pbuf);
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_osx_old_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_osx_old_callback,nwthreads,SELF_THREAD);
