...
void* ocl_bruteforce_oracle11g_thread(void *arg)
{
    int err;
...
void* ocl_markov_oracle11g_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_oracle11g_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
    ...
    if ((line[0]==0)||(ocl_rule_opt_counts[self]>=wthreads[self].vectorsize))
    {
	    ...
	    salt.s0=saltc[0]|(saltc[1]<<8)|(saltc[2]<<16)|(saltc[3]<<24);
	    salt.s1=saltc[4]|(saltc[5]<<8)|(saltc[6]<<16)|(saltc[7]<<24);
	    salt.s2=saltc[8]|(saltc[9]<<8);
	    salt.sF = 10;
...
static void ocl_oracle11g_callback(char *line, int self)
{
	...
	rule_offload_perform(ocl_oracle11g_crack_callback,self);
...
void* ocl_rule_oracle11g_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    worker_gen(self,ocl_oracle11g_callback);
...
hash_stat ocl_bruteforce_oracle11g(void)
{
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_oracle11g_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_oracle11g(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_oracle11g_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_oracle11g(void)
{
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_oracle11g_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_oracle11g_callback,nwthreads,SELF_THREAD);
