...
static void ocl_set_params(int loopnr, cl_uint4 param1, cl_uint4 param2,cl_uint16 salt,cl_uint16 plain,struct hash_list_s *list,cl_uint16 *p1, cl_uint16 *p2, cl_uint16 *p3, cl_uint16 *p4, cl_uint16 *p5, cl_uint16 *p6, cl_uint16 *p7, cl_uint16 *p8, cl_uint16 *p9, cl_uint16 *p10,struct hash_list_s *plist[4])
{
    switch (loopnr)
    {
	case 0:
...
static cl_uint16 setup_salt(char *salt)
{
    cl_uint16 t;
    char salt2[64];
...
static cl_uint16 setup_plain(char *plain)
{
    cl_uint16 t;
    char salt2[64];
    ...
    t.sC=(salt2[44]&255)|((salt2[45]&255)<<8)|((salt2[46]&255)<<16)|((salt2[47]&255)<<24);
    t.sF=(strlen(plain)+4)*8;
    return t;
}
...
static void ocl_execute(cl_command_queue queue, cl_kernel kernel, size_t *global_work_size, size_t *local_work_size, int charset_size, cl_mem found_buf, cl_mem hashes_buf, cl_mem plains_buf, char *plains, char * hashes,int self, cl_uint16 *p1,cl_uint16 *p2,cl_uint16 *p3,cl_uint16 *p4,cl_uint16 *p5,cl_uint16 *p6,cl_uint16 *p7,cl_uint16 *p8,cl_uint16 *p9,cl_uint16 *p10,struct hash_list_s *list[4])
{
    int err;
    int *found;
    int try;
    ...
    _clSetKernelArg(kernel, 11, sizeof(cl_uint16), (void*) p7);
    _clSetKernelArg(kernel, 12, sizeof(cl_uint16), (void*) p8);
    _clSetKernelArg(kernel, 13, sizeof(cl_uint16), (void*) p9);
    _clSetKernelArg(kernel, 14, sizeof(cl_uint16), (void*) p10);
...
void* ocl_bruteforce_vbulletin_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "vbulletin_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "vbulletin_long_normal", &err );
	...
	if (attack_over!=0) goto out;
        bzero(candidate,16);
        salt = setup_salt(mylist->salt);
        plain = setup_plain(candidate);
	SETUP_PARAMS();
	image.y=0x80;image.z=0;image.w=0;
	ocl_set_params(try,image,singlehash,salt,plain,mylist,&p1,&p2,&p3,&p4,&p5,&p6,&p7,&p8,&p9,&p10,plist);
	try++;
	if (try==wthreads[self].loops)
	{
	    ocl_execute(queue, kernel, global_work_size, local_work_size, charset_size, found_buf, hashes_buf, plains_buf, plains, hashes, self, &p1,&p2,&p3,&p4,&p5,&p6,&p7,&p8,&p9,&p10,plist);
	    try=0;
	}
	mylist = mylist->next;
    }
...
void* ocl_markov_vbulletin_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_vbulletin_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
...
static void ocl_vbulletin_callback(char *line, int self)
{
    rule_counts[self][0]++;
	...
	rule_offload_perform(ocl_vbulletin_crack_callback,self);
...
void* ocl_rule_vbulletin_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    rule_kernel[self] = _clCreateKernel(program[self], "vbulletin", &err );
    ...
    worker_gen(self,ocl_vbulletin_callback);
...
hash_stat ocl_bruteforce_vbulletin(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_vbulletin_long__%s.bin",DATADIR,pbuf);
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_vbulletin_long__%s.ptx",DATADIR,pbuf);
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_vbulletin_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_vbulletin(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_vbulletin_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_vbulletin(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            char kernelfile[255];
            _clGetDeviceInfo(device[wthreads[i].deviceid], CL_DEVICE_NAME, sizeof(pbuf),pbuf, NULL );
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_vbulletin__%s.bin",DATADIR,pbuf);
        ...
        else
        {
            ...
            if ((compute_capability_major==2)&&(compute_capability_minor==1)) sprintf(pbuf,"sm21");
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_vbulletin__%s.ptx",DATADIR,pbuf);
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_vbulletin_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_vbulletin_callback,nwthreads,SELF_THREAD);
