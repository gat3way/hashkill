...
static void ocl_set_params(int loopnr, cl_uint4 param1, cl_uint4 param2,cl_uint16 salt,cl_uint16 plain,struct hash_list_s *list,cl_uint16 *p1, cl_uint16 *p2, cl_uint16 *p3, cl_uint16 *p4, cl_uint16 *p5, cl_uint16 *p6, cl_uint16 *p7,struct hash_list_s *plist[4])
{
    switch (loopnr)
    {
	case 0:
...
void* ocl_bruteforce_ipb2_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "ipb2_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "ipb2_long_normal", &err );
	...
	ocl_set_params(try,image,singlehash,salt,plain,mylist,&p1,&p2,&p3,&p4,&p5,&p6,&p7,plist);
	try++;
	if (try==wthreads[self].loops)
	{
	    ocl_execute(queue, kernel, global_work_size, local_work_size, charset_size, found_buf, hashes_buf, plains_buf, plains, hashes, self, &p1,&p2,&p3,&p4,&p5,&p6,&p7,plist);
	    try=0;
	}
	mylist = mylist->next;
    }
...
void* ocl_markov_ipb2_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_ipb2_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
	...
	salt.s1=mylist->salt[4]|(0x80<<8);
	salt.sF = strlen(mylist->salt)<<3;
...
static void ocl_ipb2_callback(char *line, int self)
{
    rule_counts[self][0]++;
	...
	rule_offload_perform(ocl_ipb2_crack_callback,self);
...
void* ocl_rule_ipb2_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    rule_kernel[self] = _clCreateKernel(program[self], "ipb2", &err );
    ...
    worker_gen(self,ocl_ipb2_callback);
...
hash_stat ocl_bruteforce_ipb2(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_ipb2_long__%s.bin",DATADIR,pbuf);
	    ...
	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_ipb2_long__%s.ptx",DATADIR,pbuf);
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_ipb2_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_ipb2(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_ipb2_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_ipb2(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            char kernelfile[255];
            _clGetDeviceInfo(device[wthreads[i].deviceid], CL_DEVICE_NAME, sizeof(pbuf),pbuf, NULL );
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_ipb2__%s.bin",DATADIR,pbuf);
        ...
        else
        {
            ...
            if ((compute_capability_major==2)&&(compute_capability_minor==1)) sprintf(pbuf,"sm21");
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_ipb2__%s.ptx",DATADIR,pbuf);
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_ipb2_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_ipb2_callback,nwthreads,SELF_THREAD);
