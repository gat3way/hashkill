...
#define SETUP_PARAMS() { \
        ...
        memcpy(base64,mylist->hash,36); \
        b64_pton(base64+6, mhash, 21); \
        memcpy(&A,mhash,4); \
        memcpy(&B,mhash+4,4); \
        memcpy(&C,mhash+8,4); \
        memcpy(&D,mhash+12,4); \
...
static hash_stat recreate_hash_indexes(void)
{
    ...
    while (mylist)
    {
        b64_pton(&mylist->hash[6], hex1, 20);
...
static void ocl_get_cracked(cl_command_queue queuein,cl_mem plains_buf, char *plains, cl_mem hashes_buf, char *hashes, int numfound, int vsize, int hashlen, struct hash_list_s *list)
{
    ...
    b64_pton(list->hash+6, b64, 20);

    for (a=0;a<numfound;a++)
    for (b=0;b<vsize;b++)
    if (memcmp(b64, (char *)hashes+(a*vsize+b)*hashlen, hash_ret_len1-2) == 0)
    {
    	int flag = 0;
...
void* ocl_bruteforce_ldap_ssha_thread(void *arg)
{
    int err;
...
void* ocl_markov_ldap_ssha_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_ldap_ssha_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
	...
	_clSetKernelArg(rule_kernel[self], 5, sizeof(cl_uint4), (void*) &singlehash);
		...
		for (c=0;c<wthreads[self].vectorsize;c++)
		{
		    ...
		    b64_pton(mylist->hash+6, ubase64, 20);
		    if (memcmp(ubase64, (char *)rule_ptr[self]+(e)*hash_ret_len1, hash_ret_len1-2) == 0)
    		    {
            		int flag = 0;
...
static void ocl_ldap_ssha_callback(char *line, int self)
{
	...
	rule_offload_perform(ocl_ldap_ssha_crack_callback,self);
...
void* ocl_rule_ldap_ssha_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    worker_gen(self,ocl_ldap_ssha_callback);
...
hash_stat ocl_bruteforce_ldap_ssha(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_sha1_passsalt_long__%s.bin",DATADIR,pbuf);
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_sha1_passsalt_long__%s.ptx",DATADIR,pbuf);
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_ldap_ssha_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_ldap_ssha(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_ldap_ssha_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_ldap_ssha(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            char kernelfile[255];
            _clGetDeviceInfo(device[wthreads[i].deviceid], CL_DEVICE_NAME, sizeof(pbuf),pbuf, NULL );
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_sha1_passsalt__%s.bin",DATADIR,pbuf);
        ...
        else
        {
            ...
            if ((compute_capability_major==2)&&(compute_capability_minor==1)) sprintf(pbuf,"sm21");
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_sha1_passsalt__%s.ptx",DATADIR,pbuf);
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_ldap_ssha_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_ldap_ssha_callback,nwthreads,SELF_THREAD);
