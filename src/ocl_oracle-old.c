...
static cl_uint16 setup_salt(char *salt, char *plain)
{
    ...
    for (i=0;i<strlen(plain);i++) plain[i]=toupper(plain[i]);
    for (i=0;i<strlen(salt);i++) salt[i]=toupper(salt[i]);
    bzero(salt2,64);
    strcpy(salt2,salt);
    memcpy(salt2+strlen(salt)+4,plain,strlen(plain));
    t.s0=((salt2[0]&255)<<8)|((salt2[1]&255)<<24);
    t.s1=((salt2[2]&255)<<8)|((salt2[3]&255)<<24);
    t.s2=((salt2[4]&255)<<8)|((salt2[5]&255)<<24);
    t.s3=((salt2[6]&255)<<8)|((salt2[7]&255)<<24);
    t.s4=((salt2[8]&255)<<8)|((salt2[9]&255)<<24);
    t.s5=((salt2[10]&255)<<8)|((salt2[11]&255)<<24);
    t.s6=((salt2[12]&255)<<8)|((salt2[13]&255)<<24);
    t.s7=((salt2[14]&255)<<8)|((salt2[15]&255)<<24);
    t.s8=((salt2[16]&255)<<8)|((salt2[17]&255)<<24);
    t.s9=((salt2[18]&255)<<8)|((salt2[19]&255)<<24);
    t.sA=((salt2[20]&255)<<8)|((salt2[21]&255)<<24);
    t.sB=((salt2[22]&255)<<8)|((salt2[23]&255)<<24);
    t.sC=((salt2[24]&255)<<8)|((salt2[25]&255)<<24);
    t.sD=((salt2[26]&255)<<8)|((salt2[27]&255)<<24);
    t.sE=strlen(salt);
    t.sF=(strlen(salt)+strlen(plain)+4)*2;
    return t;
}
...
static void ocl_get_cracked(cl_command_queue queuein,cl_mem plains_buf, char *plains, cl_mem hashes_buf, char *hashes, int numfound, int vsize, int hashlen, struct hash_list_s *list)
{
    ...
    if (memcmp(list->hash, (char *)hashes+(a*vsize+b)*hashlen, hash_ret_len) == 0)
    {
    	...
    	for (c=0;c<strlen(plain);c++) plain[c]=toupper(plain[c]);
...
static void ocl_execute(cl_command_queue queue, cl_kernel kernel, size_t *global_work_size, size_t *local_work_size, int charset_size, cl_mem found_buf, cl_mem hashes_buf, cl_mem plains_buf, char *plains, char * hashes,int self, cl_uint16 *p1,cl_uint16 *p2,cl_uint16 *p3,cl_uint16 *p4,cl_uint16 *p5,cl_uint16 *p6,struct hash_list_s *list[4])
{
	...
	if (interactive_mode==1)
	{
		...
		if (*found>0) 
		{
    		    ...
    		    bzero(plains,16*2*MAXFOUND);
    		    _clEnqueueWriteBuffer(queue, plains_buf, CL_FALSE, 0, 16*2*MAXFOUND, plains, 0, NULL, NULL);
    		    // Change for other types
    		    bzero(hashes,hash_ret_len*2*MAXFOUND);
    		    _clEnqueueWriteBuffer(queue, hashes_buf, CL_FALSE, 0, hash_ret_len*2*MAXFOUND, hashes, 0, NULL, NULL);
    		    *found = 0;
...
void* ocl_bruteforce_oracle_old_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "oracle_old_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "oracle_old_long_normal", &err );

    /* Create queue */
    queue = _clCreateCommandQueue(context[self], wthreads[self].cldeviceid, 0, &err );

    // Change for other lens
    hashes  = malloc(hash_ret_len*2*MAXFOUND); 
    hashes_buf = _clCreateBuffer(context[self], CL_MEM_WRITE_ONLY, hash_ret_len*2*MAXFOUND, NULL, &err );
    plains=malloc(16*2*MAXFOUND);
    bzero(plains,16*2*MAXFOUND);
	...
	if (attack_over!=0) goto out;
        bzero(candidate,16);
        salt = setup_salt(mylist->username,candidate);
	SETUP_PARAMS();
...
void* ocl_markov_oracle_old_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_oracle_old_crack_callback(char *line, int self)
{
    int a,b,c,e,f;
    int *found;
    int err;
	...
	salt.sF=strlen(line);
	salt.s0=mylist->username[0]|(mylist->username[1]<<8)|(mylist->username[2]<<16)|(mylist->username[3]<<24);
	salt.s1=mylist->username[4]|(mylist->username[5]<<8)|(mylist->username[6]<<16)|(mylist->username[7]<<24);
	salt.s2=mylist->username[8]|(mylist->username[9]<<8)|(mylist->username[10]<<16)|(mylist->username[11]<<24);
	salt.s3=mylist->username[12]|(mylist->username[13]<<8)|(mylist->username[14]<<16)|(mylist->username[15]<<24);
	salt.s4=mylist->username[16]|(mylist->username[17]<<8)|(mylist->username[18]<<16)|(mylist->username[19]<<24);
	salt.s5=mylist->username[20]|(mylist->username[21]<<8)|(mylist->username[22]<<16)|(mylist->username[23]<<24);
	salt.s6=mylist->username[24]|(mylist->username[25]<<8)|(mylist->username[26]<<16)|(mylist->username[27]<<24);
	salt.s7=mylist->username[28]|(mylist->username[29]<<8)|(mylist->username[30]<<16)|(mylist->username[31]<<24);
	salt.sF = strlen(mylist->username);
	...
	_clFinish(rule_oclqueue[self]);
		...
		for (c=0;c<wthreads[self].vectorsize;c++)
		{
    		    ...
    		    if (memcmp(mylist->hash, (char *)rule_ptr[self]+(e)*hash_ret_len, hash_ret_len) == 0)
    		    {
                	...
                	strcat(plain,line);
                	for (f=0;f<strlen(plain);f++) plain[f]=toupper(plain[f]);
                	pthread_mutex_lock(&crackedmutex);
                	addlist = cracked_list;
                	while (addlist)
                	{
...
static void ocl_oracle_old_callback(char *line, int self)
{
	...
	rule_offload_perform(ocl_oracle_old_crack_callback,self);
...
void* ocl_rule_oracle_old_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    ocl_rule_workset[self]=256*128*2;
    ...
    rule_kernel[self] = _clCreateKernel(program[self], "oracle_old", &err );
    ...
    rule_images2_buf[self] = _clCreateBuffer(context[self], CL_MEM_READ_WRITE, ocl_rule_workset[self]*wthreads[self].vectorsize*56, NULL, &err );
    ...
    rule_images2[self]=malloc(ocl_rule_workset[self]*wthreads[self].vectorsize*56);
    bzero(&rule_images[self][0],ocl_rule_workset[self]*wthreads[self].vectorsize*MAX);
    ...
    worker_gen(self,ocl_oracle_old_callback);
...
hash_stat ocl_bruteforce_oracle_old(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_oracle_old_long__%s.bin",DATADIR,pbuf);
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_oracle_old_long__%s.ptx",DATADIR,pbuf);
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_oracle_old_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_oracle_old(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_oracle_old_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_oracle_old(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            char kernelfile[255];
            _clGetDeviceInfo(device[wthreads[i].deviceid], CL_DEVICE_NAME, sizeof(pbuf),pbuf, NULL );
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_oracle_old__%s.bin",DATADIR,pbuf);
        ...
        else
        {
            ...
            if ((compute_capability_major==2)&&(compute_capability_minor==1)) sprintf(pbuf,"sm21");
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_oracle_old__%s.ptx",DATADIR,pbuf);
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_oracle_old_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_oracle_old_callback,nwthreads,SELF_THREAD);
