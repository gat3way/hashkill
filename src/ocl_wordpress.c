...
static void ocl_wordpress_callback(char *line, int self)
{
    rule_counts[self][0]++;
	...
	rule_offload_perform(ocl_wordpress_crack_callback,self);
...
void* ocl_rule_wordpress_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    rule_kernel[self] = _clCreateKernel(program[self], "phpass", &err );
    ...
    worker_gen(self,ocl_wordpress_callback);
...
hash_stat ocl_rule_wordpress(void)
{
    ...
    for (a=0;a<nwthreads;a++)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_rule_wordpress_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_wordpress_callback,nwthreads,SELF_THREAD);
