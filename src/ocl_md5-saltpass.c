...
void* ocl_bruteforce_md5_saltpass_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "md5_saltpass_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "md5_saltpass_long_normal", &err );
...
void* ocl_markov_md5_saltpass_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_md5_saltpass_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
...
static void ocl_md5_saltpass_callback(char *line, int self)
{
    rule_counts[self][0]++;
	...
	rule_offload_perform(ocl_md5_saltpass_crack_callback,self);
...
void* ocl_rule_md5_saltpass_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    rule_kernel[self] = _clCreateKernel(program[self], "md5_saltpass", &err );
    ...
    worker_gen(self,ocl_md5_saltpass_callback);
...
hash_stat ocl_bruteforce_md5_saltpass(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_md5_saltpass_long__%s.bin",DATADIR,pbuf);
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_md5_saltpass_long__%s.ptx",DATADIR,pbuf);
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_md5_saltpass_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_md5_saltpass(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_md5_saltpass_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_md5_saltpass(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            char kernelfile[255];
            _clGetDeviceInfo(device[wthreads[i].deviceid], CL_DEVICE_NAME, sizeof(pbuf),pbuf, NULL );
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_md5_saltpass__%s.bin",DATADIR,pbuf);
        ...
        else
        {
            ...
            if ((compute_capability_major==2)&&(compute_capability_minor==1)) sprintf(pbuf,"sm21");
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_md5_saltpass__%s.ptx",DATADIR,pbuf);
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_md5_saltpass_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_md5_saltpass_callback,nwthreads,SELF_THREAD);
