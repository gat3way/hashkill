...
/* Function prototypes */
hash_stat hash_proto_md5(char *  plaintext[VECTORSIZE], char *  hash[VECTORSIZE], int len, int threadid);
void hash_proto_md5_hex(const char *hash[VECTORSIZE], char *hashhex[VECTORSIZE]);
hash_stat hash_proto_sha1(char *plaintext[VECTORSIZE], char *hash[VECTORSIZE], int len, int threadid);
...
hash_stat hash_proto_md5(char *  plaintext[VECTORSIZE], char *  hash[VECTORSIZE], int len, int threadid)
{
	...
	if (lensflag == 0) return OMD5_SHORT((unsigned char **)plaintext, lens, (unsigned char **)hash);
	else return OMD5((unsigned char **)plaintext, lens, (unsigned char **)hash);
    }
    else 
    {
	if (len<12)
	{
	    return OMD5_SHORT_FIXED((unsigned char **)plaintext, len, (unsigned char **)hash);
	}
	else 
	{
	    return OMD5_FIXED((unsigned char **)plaintext, len, (unsigned char **)hash);
	}
    }
    return hash_ok;
#endif
...
/* MD4() proto function */
hash_stat hash_proto_md4(char *plaintext[VECTORSIZE], char * hash[VECTORSIZE], int len[VECTORSIZE],int threadid)
{
	...
	if (lensflag == 0) return OMD4_SHORT(plaintext, hash,len);
	else return OMD4(plaintext, hash,len);
	return hash_ok;
    }
    else 
    {
	if (threadid>15) return OMD4_FIXED(plaintext, hash,threadid);
	else return OMD4_SHORT_FIXED(plaintext, hash,threadid);
	return hash_ok;
    }
...
/* MD4() proto function */
hash_stat hash_proto_md4_unicode(char *plaintext[VECTORSIZE], char * hash[VECTORSIZE], int len,int threadid)
{
    ...
    {
	MD4_Init(&ctx);
	MD4_Update(&ctx, plaintext[a], len);
	MD4_Final((unsigned char *)hash[a],&ctx);
    }
    return hash_ok;
#else
    if (len>15) return OMD4_FIXED(plaintext, hash,len);
    else return OMD4_SHORT_FIXED(plaintext, hash,len);
#endif
}
...
hash_stat hash_proto_sha1(char *plaintext[VECTORSIZE], char *hash[VECTORSIZE], int len, int threadid)
{
	...
	if (lensflag == 0) return OSHA1_SHORT((char **)plaintext, (char **)hash, lens);
	else return OSHA1((char **)plaintext, (char **)hash, lens);
	return hash_ok;
    }
    else 
    {
	if (len>15) return OSHA1_FIXED((char **)plaintext, (char **)hash, len);
	else return OSHA1_SHORT_FIXED((char **)plaintext, (char **)hash, len);
	return hash_ok;
    }
#endif
}
...
hash_stat hash_proto_fcrypt(const char *password[VECTORSIZE], const char *salt, char *ret[VECTORSIZE])
{
    ...
    return hash_ok;
#else
    return ODES_FCRYPT((char *)salt, (char **)password, ret);
#endif
} 
...
hash_stat add_hash_list(char *username, char *hash, char *salt, char *salt2)
{
	...
	temp_list->indexnext = NULL;
	temp_list->indexprev = NULL;
	single_hash = 0;
	...
	hash_list->next = NULL;
	hash_list->prev = NULL;
	hash_list->indexnext = NULL;
	hash_list->indexprev = NULL;
	single_hash = 1;
    ...
    if (username)
    {
	temp_list->username = malloc(MAX_USERNAME);
	bzero(temp_list->username,MAX_USERNAME);
    ...
    if (salt2)
    {
	//posix_memalign((void **)&temp_list->salt2,64,HASHFILE_MAX_PLAIN_LENGTH );
...
void print_cracked_list(void)
{
	...
	*/
    ...
    pthread_mutex_unlock(&crackedmutex);
    printf("\n");
    hlog("Total: %d passwords cracked\n",cracked);
}
...
{
    struct hash_list_s *mylist,*mylist2;
    int a,b;
    ...
    while (mylist)
    {
        if (hash_index[mylist->hash[0]&255][mylist->hash[1]&255].nodes==NULL)
        {
    	    hash_index[mylist->hash[0]&255][mylist->hash[1]&255].nodes = mylist;
    	}
    	else
    	{
    	    mylist2 = hash_index[mylist->hash[0]&255][mylist->hash[1]&255].nodes;
    	    while (mylist2->indexnext) mylist2 = mylist2->indexnext;
    	    mylist2->indexnext = mylist;
    	    mylist->indexprev = mylist2->indexprev;
    	}
        if (mylist) mylist = mylist->next;
    }
    return hash_ok;
...
static int scheduler_check1()
{
    if (scheduler.bitmap1>=scheduler.charset_size) return 1;
    return 0;
}
...
{
    if (attack_method==attack_method_markov) {if (scheduler.bitmap2[scheduler.markov_l2_1]>=scheduler.markov_l2_2) return 1;}
    else if (scheduler.bitmap2[scheduler.charset_size-1]>=scheduler.charset_size2) return 1;
    return 0;
}

/* Scheduler - check bitmap coverage - case 3) */
static int scheduler_check3()
{
    if (attack_method==attack_method_markov) {if (scheduler.bitmap3[scheduler.markov_l3_1][scheduler.markov_l3_2]>=scheduler.markov_l3_3) return 1;}
    else if (scheduler.bitmap3[scheduler.charset_size-1][scheduler.charset_size2-1]>=scheduler.charset_size2) return 1;
    return 0;
}
...
static void *scheduler_thread_worker(void *arg)
{
    ...
    {
	check=0;
	if (scheduler.len==scheduler.startlen) check = scheduler_check1();
	if (scheduler.len==scheduler.startlen+1) check = scheduler_check2();
	if (scheduler.len>scheduler.startlen+1) check = scheduler_check3();
	if (check==1) 
	{
	    scheduler_reset_bitmap();
	    scheduler.len=scheduler.len+1;
	    //printf("nextlen=%d\n",scheduler.len);
	}
	/* Sleep */
	usleep(10000);
...
void scheduler_setup(int curlen, int startlen, int maxlen, int charset_size, int charset_size2)
{
    ...
    {
	scheduler.bitmap2[a]=0;
	scheduler.ebitmap2[a]=charset_size2;
    }
    scheduler.bitmap1=0;
    scheduler.ebitmap1=charset_size;
