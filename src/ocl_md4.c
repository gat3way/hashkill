...
void* ocl_bruteforce_md4_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "md4_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "md4_long_normal", &err );
	...
	memcpy(hex1,mylist->hash+4,4);
        ...
        REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);    //w7
        REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
        REVERSE_MD4STEP_ROUND3 (A, B, C, D, 0,      S31);    //w3
        REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
        REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);    //w5
        REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
        REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.y,S31);    //w1
        REVERSE_MD4STEP_ROUND3 (B, C, D, A, 32,     S34);    //SIZE
        REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);
        REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
        REVERSE_MD4STEP_ROUND3 (A, B, C, D, 0,      S31);    //w2
        REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
        REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);    //w4
        REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
        REVERSE_MD4STEP_ROUND3 (A, B, C, D, 0,      S31);    //w0
	...
	if (!hash_list->next)
	{
	    ...
	    memcpy(hex1,mylist->hash+12,4);
            ...
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.y,S31);    //w1
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 40,     S34);    //SIZE
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);    //w6
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, 0,      S31);    //w2
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);    //w4
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.y,S31);    //w1
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 48,     S34);    //SIZE
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);    //w6
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, 0,      S31);    //w2
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
            ...
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, 0,      S31);    //w0
            singlehash.x=A;singlehash.y=B;singlehash.z=C;singlehash.w=D;    	    ocl_set_params(try,image,singlehash,&p1,&p2,&p3,&p4);
        }
	else
	{
    	    image.x=0;
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.y,S31);    //w1
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 56,     S34);    //SIZE
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);    //w6
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, 0,      S31);    //w2
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.y,S31);    //w1
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 64,     S34);    //SIZE
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);    //w6
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.z,S31);    //w2
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);    //w4
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, 0,      S31);    //w0
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.y,S31);    //w1
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 72,     S34);    //SIZE
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);    //w6
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.z,S31);    //w2
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.y,S31);    //w1
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 80,     S34);    //SIZE
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);    //w6
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.z,S31);    //w2
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.y,S31);    //w1
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 88,     S34);    //SIZE
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);    //w6
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.z,S31);    //w2
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);    //w7
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.w,S31);    //w3
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);    //w5
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.y,S31);    //w1
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 96,     S34);    //SIZE
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);    //w6
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.z,S31);    //w2
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.y,S31);    //w1
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 104,    S34);    //SIZE
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);    //w6
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.z,S31);    //w2
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.y,S31);    //w1
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 112,    S34);    //SIZE
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);    //w6
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.z,S31);    //w2
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
	...
	if (attack_over!=0) goto out;
        ...
        if (!hash_list->next)
        {
            ...
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.y,S31);    //w1
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 120,    S34);    //SIZE
            REVERSE_MD4STEP_ROUND3 (C, D, A, B, 0,      S33);
            REVERSE_MD4STEP_ROUND3 (D, A, B, C, 0,      S32);    //w6
            REVERSE_MD4STEP_ROUND3 (A, B, C, D, image.z,S31);    //w2
            REVERSE_MD4STEP_ROUND3 (B, C, D, A, 0,      S34);
...
void* ocl_markov_md4_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_md4_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
    ...
    if ((line[0]==0)||(ocl_rule_opt_counts[self]>=wthreads[self].vectorsize))
    {
        ...
        if (*found>0) 
        {
            ...
            if (rule_found_ind[self][a]==1)
            {
                ...
                for (c=0;c<ocl_rule_opt_counts[self];c++)
                {
...
static void ocl_md4_callback(char *line, int self)
{
    rule_counts[self][0]++;
	...
	ocl_rule_opt_counts[self]=0;
	rule_offload_perform(ocl_md4_crack_callback,self);
...
void* ocl_rule_md4_thread(void *arg)
{
    cl_int err;
    unsigned char hex1[hash_ret_len];
    int a;
    ...
    rule_kernel[self] = _clCreateKernel(program[self], "md4", &err );
    ...
    if (hash_list->next) rule_bitmaps_buf[self] = _clCreateBuffer(context[self], CL_MEM_READ_ONLY|CL_MEM_USE_HOST_PTR, 256*256*32*4*8, bitmaps, &err );
    else rule_bitmaps_buf[self] = _clCreateBuffer(context[self], CL_MEM_READ_ONLY, 4, NULL, &err );
    ...
    worker_gen(self,ocl_md4_callback);
...
hash_stat ocl_bruteforce_md4(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    if (hash_list->next) 
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/amd_md4_long__%s.bin",DATADIR,pbuf);
    	    }
    	    else
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/amd_md4_long_S_%s.bin",DATADIR,pbuf);
	    }
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    if (hash_list->next) 
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/nvidia_md4_long__%s.ptx",DATADIR,pbuf);
    	    }
    	    else
    	    {
    	        sprintf(kernelfile,"%s/hashkill/kernels/nvidia_md4_long_S_%s.ptx",DATADIR,pbuf);
	    }
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_md4_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_md4(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_md4_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_md4(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            if (hash_list->next) 
            {
        	sprintf(kernelfile,"%s/hashkill/kernels/amd_md4__%s.bin",DATADIR,pbuf);
    	    }
    	    else
    	    {
        	sprintf(kernelfile,"%s/hashkill/kernels/amd_md4_S_%s.bin",DATADIR,pbuf);
	    }
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
            if (hash_list->next) 
            {
        	sprintf(kernelfile,"%s/hashkill/kernels/nvidia_md4__%s.ptx",DATADIR,pbuf);
    	    }
    	    else
    	    {
        	sprintf(kernelfile,"%s/hashkill/kernels/nvidia_md4_S_%s.ptx",DATADIR,pbuf);
	    }
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_md4_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_md4_callback,nwthreads,SELF_THREAD);
