...
/* Base64 conversion */
static int b64_pton(char const *src, char *target, int targsize)
{
...
static hash_stat recreate_hash_indexes(void)
{
    ...
    while (mylist)
    {
        b64_pton(&mylist->hash[5], hex1, 20);
...
static void ocl_get_cracked(cl_command_queue queuein,cl_mem plains_buf, char *plains, cl_mem hashes_buf, char *hashes, int numfound, int vsize, int hashlen)
{
	...
	while (mylist)
	{
            b64_pton(&mylist->hash[5], base64, 20);
    	    if (memcmp(base64, (char *)hashes+(e)*hashlen, hash_ret_len1-2) == 0)
    	    {
        	int flag = 0;
        	...
        	while (addlist)
        	{
            	    if ( memcmp(addlist->hash, mylist->hash, hash_ret_len1) == 0)
                        flag = 1;
...
void* ocl_bruteforce_nsldap_thread(void *arg)
{
    ...
    if (a==1)
    {
        ...
        mylist = hash_list;
        memcpy(base64,mylist->hash,35);
        b64_pton(&base64[5], hex1, 21);
        memcpy(&singlehash.x, hex1, 4);
        REV(singlehash.x);
        singlehash.x-=H0;
        memcpy(&singlehash.y, hex1+4, 4);
        REV(singlehash.y);
        singlehash.y-=H1;
        memcpy(&singlehash.z, hex1+8, 4);
        REV(singlehash.z);
        singlehash.z-=H2;
        memcpy(&singlehash.w, hex1+12, 4);
        REV(singlehash.w);
        singlehash.w-=H3;
        memcpy(&E,hex1+16,4);
        REV(E);
	...
	if (*found>0) 
	{
		...
		for (c=0;c<wthreads[self].vectorsize;c++)
		{
	    	    e=(a)*wthreads[self].vectorsize+c;
    	    		...
    	    		{
    			    b64_pton(&mylist->hash[5], base64, 20);
    			    if (memcmp(base64, (char *)rule_ptr[self]+(e)*hash_ret_len1, hash_ret_len1-2) == 0)
    			    {
                    		int flag = 0;
...
void* ocl_rule_nsldap_thread(void *arg)
{
    ...
    if (a==1)
    {
        unsigned int E,tmp,tmp1,tmp2;
        mylist = hash_list;
        memcpy(base64,mylist->hash,34);
        b64_pton(&base64[5], hex1, 21);
        memcpy(&rule_singlehash[self].x, hex1, 4);
        memcpy(&rule_singlehash[self].y, hex1+4, 4);
        memcpy(&rule_singlehash[self].z, hex1+8, 4);
        memcpy(&rule_singlehash[self].w, hex1+12, 4);
        memcpy(&E,hex1+16,4);
