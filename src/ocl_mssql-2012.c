...
static void ocl_set_params(int loopnr, cl_uint4 param1, cl_ulong4 param2,cl_uint16 salt,struct hash_list_s *list,cl_uint16 *p1, cl_ulong8 *p2, cl_uint16 *p3, cl_uint16 *p4,struct hash_list_s *plist[2])
{
    switch (loopnr)
    {
	case 0:
...
static void ocl_execute(cl_command_queue queue, cl_kernel kernel, size_t *global_work_size, size_t *local_work_size, int charset_size, cl_mem found_buf, cl_mem hashes_buf, cl_mem plains_buf, char *plains, char * hashes,int self, cl_uint16 *p1,cl_ulong8 *p2,cl_uint16 *p3,cl_uint16 *p4,struct hash_list_s *list[2])
{
    int err;
    int *found;
    int try;
    ...
    _clSetKernelArg(kernel, 6, sizeof(cl_long8), (void*) p2);
...
void* ocl_bruteforce_mssql_2012_thread(void *arg)
{
    int err;
    ...
    /* Init kernels */
    if (ocl_gpu_double) kernel = _clCreateKernel(program[self], "sha512_passsalt_long_double", &err );
    else  kernel = _clCreateKernel(program[self], "sha512_passsalt_long_normal", &err );
	...
	ocl_set_params(try,image,singlehash,salt,mylist,&p1,&p2,&p3,&p4,plist);
	try++;
	if (try==wthreads[self].loops)
	{
	    ocl_execute(queue, kernel, global_work_size, local_work_size, charset_size, found_buf, hashes_buf, plains_buf, plains, hashes, self, &p1,&p2,&p3,&p4,plist);
	    try=0;
	}
	mylist = mylist->next;
    }
...
void* ocl_markov_mssql_2012_thread(void *arg)
{
    int err;
    cl_command_queue queue;
    cl_mem hashes_buf;
...
/* Crack callback */
static void ocl_mssql_2012_crack_callback(char *line, int self)
{
    int a,b,c,e;
    int *found;
    int err;
    ...
    if ((line[0]==0)||(ocl_rule_opt_counts[self]>=wthreads[self].vectorsize))
    {
	    ...
	    salt.s2=salt.s3=salt.s4=salt.s5=salt.s6=salt.s7=salt.s8=salt.s9=salt.sA=salt.sB=salt.sC=salt.sD=salt.sE=0;
            ...
            _clSetKernelArg(rule_kernel[self], 5, sizeof(cl_ulong4), (void*) &rule_singlehash_long[self]);
...
static void ocl_mssql_2012_callback(char *line, int self)
{
	...
	ocl_rule_opt_counts[self]=0;
	rule_offload_perform(ocl_mssql_2012_crack_callback,self);
...
void* ocl_rule_mssql_2012_thread(void *arg)
{
    cl_int err;
    int found;
    ...
    rule_counts[self][0]=-1;
    rule_kernel[self] = _clCreateKernel(program[self], "sha512_passsalt", &err );
    ...
    worker_gen(self,ocl_mssql_2012_callback);
...
hash_stat ocl_bruteforce_mssql_2012(void)
{
    ...
    for (i=0;i<nwthreads;i++) if (wthreads[i].type!=cpu_thread)
    {
    	...
    	if (wthreads[i].type != nv_thread)
    	{
    	    ...
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_sha512_passsaltu_long__%s.bin",DATADIR,pbuf);
	    ...
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_sha512_passsaltu_long__%s.ptx",DATADIR,pbuf);
    ...
    for (a=0;a<nwthreads;a++) if (wthreads[a].type!=cpu_thread)
    {
        ...
        pthread_create(&crack_threads[a], NULL, ocl_bruteforce_mssql_2012_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_markov_mssql_2012(void)
{
    ...
    {
	worker_thread_keys[a]=a;
	pthread_create(&crack_threads[a], NULL, ocl_markov_mssql_2012_thread, &worker_thread_keys[a]);
    }
...
hash_stat ocl_rule_mssql_2012(void)
{
	...
	_clGetDeviceIDs(platform[wthreads[i].platform], CL_DEVICE_TYPE_GPU, 64, device, (cl_uint *)&devicesnum);
        ...
        if (wthreads[i].type != nv_thread)
        {
            ...
            char kernelfile[255];
            _clGetDeviceInfo(device[wthreads[i].deviceid], CL_DEVICE_NAME, sizeof(pbuf),pbuf, NULL );
    	    sprintf(kernelfile,"%s/hashkill/kernels/amd_sha512_passsaltu__%s.bin",DATADIR,pbuf);
        ...
        else
        {
            ...
            if ((compute_capability_major==2)&&(compute_capability_minor==1)) sprintf(pbuf,"sm21");
	    if ((compute_capability_major==3)&&(compute_capability_minor==0)) sprintf(pbuf,"sm30");
    	    sprintf(kernelfile,"%s/hashkill/kernels/nvidia_sha512_passsaltu__%s.ptx",DATADIR,pbuf);
    ...
    {
        worker_thread_keys[a]=a;
        pthread_create(&crack_threads[a], NULL, ocl_rule_mssql_2012_thread, &worker_thread_keys[a]);
    }
    rule_gen_parse(rule_file,ocl_mssql_2012_callback,nwthreads,SELF_THREAD);
