...
[
 ...
 AC_CACHE_CHECK([whether sse2 is supported], [ax_have_sse2_ext_cv_],
  [
    ax_have_sse2_ext_cv_=no
    if test "$((0x$edx>>26&0x01))" = 1; then
      ax_have_sse2_ext_cv_=yes
    fi
  ])

  if test "$ax_have_sse2_ext_cv_" = yes; then
    AC_DEFINE(HAVE_SSE2, , [Support SSE2 (Streaming SIMD Extensions 2) instructions])
    export DEPS_CFLAGS="-msse2"
