...
AC_DEFUN([AX_CHECK_CL],
[AC_MSG_CHECKING(if opencl is enabled)
AC_ARG_WITH(opencl,
[  --with-opencl=DIR	] [ path to OpenCL SDK installation]
[  --without-opencl 	  to disable opencl usage completely],
  [if test "$withval" != no ; then
  AC_MSG_RESULT(yes)
  if test -d "$withval"
  then
     ATISTREAMSDKROOT=$withval
  else
...
AS_IF([test X$no_x != Xyes],
      [AS_IF([test -n "$x_includes"],
       ...
       AS_IF([test -n "$x_libraries"],
             [CL_LIBS="$CL_LIBS -DHAVE_OPENCL"]))

ax_save_CPPFLAGS=$CPPFLAGS
CL_CFLAGS="$CL_CFLAGS -I$ATISTREAMSDKROOT/include -L$ATISTREAMSDKROOT/lib/x86_64 -L$ATISTREAMSDKROOT/lib/x86 -I$ATISTREAMSDKROOT/include -I$ATISTREAMSDKROOT/OpenCL/common/inc"
CL_LIBS="$CL_LIBS -L$ATISTREAMSDKROOT/lib/x86_64 -L$ATISTREAMSDKROOT/lib/x86 -L/usr/lib"
